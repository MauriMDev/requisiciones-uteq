...
class SolicitudesController {
    ...
    async obtenerSolicitudes(req, res) {
        try {
            ...
            const solicitudesConContadores = solicitudes.map(solicitud => {
                ...
                const aprobacionesPositivas = solicitudData.aprobaciones.filter(a => a.accion === 'aprobar').length;
    ...
    async obtenerSolicitudPorId(req, res) {
        try {
                ...
                include: [
                    {
                        model: Usuario,
                        as: 'solicitante',
                        attributes: ['nombre_completo', 'numero_empleado', 'correo_institucional', 'telefono']
                    },
                    {
                        model: Departamento,
                        as: 'departamento',
            ...
            // Control de acceso
            if (req.user.rol === 'solicitante' && solicitud.solicitante_id !== req.user.id_usuario) {
    ...
    async actualizarSolicitud(req, res) {
        ...
        try {
            ...
            if (tipo_requisicion !== undefined) updateData.tipo_requisicion = tipo_requisicion;
            if (descripcion_detallada !== undefined) updateData.descripcion_detallada = descripcion_detallada;
            ...
            if (presupuesto_estimado !== undefined) updateData.presupuesto_estimado = presupuesto_estimado;
            if (fecha_necesidad !== undefined) updateData.fecha_necesidad = fecha_necesidad;
            if (items !== undefined) updateData.items = items;
            if (comentarios_generales !== undefined) updateData.comentarios_generales = comentarios_generales;
    ...
    async obtenerEstadisticas(req, res) {
        try {
            ...
            } else if (departamento_id && ['admin_sistema', 'administrativo'].includes(req.user.rol)) {
            ...
            const estadisticas = await sequelize.query(`
                ...
                WHERE ${Object.keys(whereConditions).length > 0 ? 
                    Object.keys(whereConditions).map((key, index) => {
                        if (key === 'fecha_creacion') {
                            let condition = '';
                            if (whereConditions[key][Op.gte]) condition += `DATE(fecha_creacion) >= :fecha_inicio`;
                            if (whereConditions[key][Op.lte]) condition += `${condition ? ' AND ' : ''}DATE(fecha_creacion) <= :fecha_fin`;
                            return condition;
                        }
                        return `${key} = :${key}`;
                    }).join(' AND ') : '1=1'
            ...
            `, {
                ...
                replacements: {
                    ...
                    solicitante_id: req.user.rol === 'solicitante' ? req.user.id_usuario : undefined
                }
            });
